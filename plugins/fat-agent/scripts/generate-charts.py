...
try:
    ...
    # import matplotlib.ticker as mticker  # unused
    ...
    for candidate in [
        ...
        try:
            ...
            matches = [
                ...
                if candidate.lower().replace(" ", "") in f.lower().replace(" ", "")
            ]
            if matches:
...
def chart_fat_scores(scores, output_dir, dpi=200):
    ...
    ax1.set_xlim(0, 105)
    ax1.set_title(
        "FAT Agent Scores", fontsize=15, fontweight="bold", color=GRAY, pad=12
    )
    ...
    if total > 0:
        ...
        filtered = [
            (s, lbl, c) for s, lbl, c in zip(sizes, labels, issue_colors) if s > 0
        ]
        if filtered:
...
def chart_pagespeed(pagespeed, output_dir, dpi=200):
    ...
    for ax, data, title, title_color in [
        ...
        bars = ax.bar(
            categories, data, color=colors, width=0.6, edgecolor=WHITE, linewidth=0.5
        )
    ...
    fig.suptitle(
        "PageSpeed Insights Scores", fontsize=15, fontweight="bold", color=GRAY, y=1.02
    )
    plt.tight_layout()
...
def chart_traffic_trend(semrush, output_dir, dpi=200):
    ...
    ax.set_title(
        f'Organic Traffic Trend{" — " + domain if domain else ""}',
        fontsize=16,
        fontweight="bold",
        color=GRAY,
        pad=15,
    ...
    ax.annotate(
        ...
        color=GREEN,
        fontweight="bold",
        xytext=(peak_idx, peak_val * 1.08),
        ha="center",
    )
    if change:
        ax.annotate(
            f"Current: {current_val:,}\n({change})",
            xy=(len(months) - 1, current_val),
            fontsize=9,
            color=RED,
            fontweight="bold",
            xytext=(len(months) - 1.8, current_val * 0.5),
            ha="center",
...
def chart_keywords_trend(semrush, output_dir, dpi=200):
    ...
    fig, axes = plt.subplots(
        1,
        ncols,
        figsize=(12 if ncols == 2 else 7, 4.5),
    ...
    if has_trend:
        ...
        ax.set_title(
            "Organic Keywords Trend", fontsize=14, fontweight="bold", color=GRAY, pad=12
        )
        ...
        if change:
            ax.annotate(
                f"Current: {current:,}\n({change})",
                xy=(len(months) - 1, current),
                fontsize=9,
                color=RED,
                fontweight="bold",
                xytext=(len(months) - 2, current * 0.6),
                ha="center",
                arrowprops=dict(arrowstyle="->", color=RED, lw=1.5),
            )
...
def chart_top_keywords(semrush, output_dir, dpi=200):
    ...
    bars = ax.barh(
        y_pos, volumes, color=colors, height=0.6, edgecolor=WHITE, linewidth=0.5
    )
    ...
    ax.set_title(
        "Top Ranking Keywords by Search Volume",
        fontsize=15,
        fontweight="bold",
        color=GRAY,
        pad=15,
    ...
    legend_elements = [
        Patch(facecolor=GREEN, label="Position 1-3"),
...
def chart_overview(semrush, output_dir, dpi=200):
    ...
    metrics = [
        (
            "Authority\nScore",
            str(semrush.get("authority_score", "?")),
            "Average" if semrush.get("authority_score", 0) < 40 else "Good",
            ORANGE,
        ),
        (
            "Organic\nTraffic",
            str(semrush.get("organic_traffic", "?")),
            semrush.get("traffic_change", ""),
            RED if "-" in str(semrush.get("traffic_change", "")) else GREEN,
        ),
        (
            "Keywords",
            str(semrush.get("organic_keywords", "?")),
            semrush.get("keywords_change", ""),
            YELLOW,
        ),
        ...
        (
            "Traffic\nCost",
            (
                f"${semrush.get('traffic_cost', '?'):,}"
                if isinstance(semrush.get("traffic_cost"), (int, float))
                else str(semrush.get("traffic_cost", "?"))
            ),
            "USD/mo",
            GREEN,
        ),
    ...
    fig.suptitle(
        f"SEMrush Domain Overview  —  {domain}",
        fontsize=13,
        fontweight="bold",
        color=WHITE,
        y=1.02,
...
def main():
    ...
    parser.add_argument(
        "--charts", help="Comma-separated chart names (default: all available)"
    )
