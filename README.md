...
- **Google Guidelines** — checks pages and whole sites against Google's spam policies and 2024-2026 guidance: templated **doorway / scaled-content clusters** (simhash over main content, triaged keep/improve/prune with Search Console data), keyword and location stuffing, hidden text, sneaky redirects, **back button hijacking** (enforced Jun 2026), `noindex` in `<body>`, the 2MB HTML limit, retired rich results (FAQ gone May 2026), title/meta hygiene. `update_impact.py` lines traffic up against every core/spam update.
- **Model judgments (Jev)** — `jev.py` answers the judgment calls regexes can't ("is this suburb page real local content or the template with the name swapped?", "does this page actually answer this question?") with calibrated yes/no probabilities. It works with **no setup** (the agent answers a sample), with a **TypeSafe Jev key**, with a **free hosted Jev-compatible API** such as Codiv, or with **OpenJev on your own GPU**. No key is ever required.
- **Query fan-out** — `fanout.py` builds the sub-queries AI Mode and AI Overviews fan out to and scores whether the site answers each with a page or section, then hands the set to the live citation checker.
...
- **tests/** — 915 tests across 55 test files with fixtures
...
915 tests covering all modules, the registry, crawlers (incl. a live local-server site crawl), bulk audit, Lighthouse, visual regression, dashboard, CI gate, client-facing transforms, profiles, and the persistent punch list.
